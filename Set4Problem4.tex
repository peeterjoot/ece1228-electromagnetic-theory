...
\begin{equation}\label{eqn:emtProblemSet4Problem4:20}
\BJ = \sigma \BE,
\end{equation}
then the continuity equation can be written as
%
\begin{equation}\label{eqn:emtProblemSet4Problem4:40}
\begin{aligned}
\PD{t}{\rho}
&=
-\spacegrad \cdot \BJ
...
%\begin{equation}\label{eqn:emtProblemSet4Problem4:60}
...
%\sigma = 6.3 * 10^7
Given this dispersion relation for the charge density, we can also find the normal component of the current density using the divergence theorem.  Evaluating the continuity equation in an infinitesimal spherical volume of radius \( R \) surrounding the initial charge \( \rho_0 dV \) that was placed at the center of that sphere, far enough from the boundary of the conductor that we can ignore it, we find
\begin{equation}\label{eqn:emtProblemSet4Problem4:100}
\oint \ncap \cdot \BJ dA = - \PD{t}{} \int \rho_0 e^{-\sigma t/\epsilon} dV,
\end{equation}
or
\begin{equation}\label{eqn:emtProblemSet4Problem4:120}
